...
public class RestartServer extends BukkitRunnable implements CommandExecutor{
    ...
    public RestartServer(){
        Bukkit.broadcast(Component.text(Main.getInstance().getConfig().get("messages.prefix")+"Le serveur redémarre dans §e%s §7secondes".formatted(seconds)));
    ...
    public void run() {
        if(seconds >0 && seconds <= 5){
            Bukkit.broadcast(Component.text(Main.getInstance().getConfig().get("messages.prefix")+"Le serveur redémarre dans §e%s §7seconde%s".formatted(seconds, seconds == 1 ? "" : "s")));
            for(Player player : Bukkit.getOnlinePlayers()){
                player.playSound(player.getLocation(), Sound.ENTITY_EXPERIENCE_ORB_PICKUP, 1, 1);
            }
        }else if(seconds ==0){
            ...
            for(Player player : Bukkit.getOnlinePlayers()){
                player.kick(Component.text(
                        "\n§8§m----------------------------------------\n" +
                                "§r" + Main.getInstance().getConfig().get("messages.prefix") + "\n" +
                                "§r\n" +
